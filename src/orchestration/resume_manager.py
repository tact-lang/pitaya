...
def _backfill_terminal(orchestrator, run_id: str) -> None:
    try:
        term_types = {"task.completed", "task.failed", "task.interrupted"}
        seen: set[str] = set()
        try:
            events, _ = orchestrator.event_bus.get_events_since(offset=0, event_types=term_types)
        except Exception:
            events = []
        for ev in events or []:
        ...
        for iid, info in list(orchestrator.state_manager.current_state.instances.items()):
...
def _reopen_strategies(orchestrator, run_id: str) -> None:
    try:
        for sid, strat in list(orchestrator.state_manager.current_state.strategies.items()):
...
async def _schedule_instances(orchestrator, saved_state) -> List[str]:
    ...
    for iid, info in saved_state.instances.items():
        try:
            if info.state in (InstanceStatus.RUNNING, InstanceStatus.INTERRUPTED):
                ...
                plugin_cls = AVAILABLE_PLUGINS.get(plugin_name) or AVAILABLE_PLUGINS["claude-code"]
                plugin = plugin_cls()
                can_resume = bool(info.session_id and plugin.capabilities.supports_resume)
                if can_resume:
                    orchestrator.state_manager.update_instance_metadata(
                        iid, {"reuse_container": True, "operator_resume": True}
                    )
                    orchestrator.state_manager.update_instance_state(iid, InstanceStatus.QUEUED)
                    await _enqueue(iid)
                    ...
                    try:
                        ...
                        orchestrator.state_manager.update_instance_container_name(iid, new_name)
                        meta_patch["container_name_override"] = new_name
                        orchestrator.state_manager.update_instance_metadata(iid, meta_patch)
                    except Exception:
                        pass
                    orchestrator.state_manager.update_instance_state(iid, InstanceStatus.QUEUED)
                    await _enqueue(iid)
                    scheduled.append(iid)
                    logger.info("resume_run: scheduled fresh iid=%s (no resumable session)", iid)
...
async def _reenter_strategies(orchestrator, run_id: str) -> List[InstanceResult]:
    ...
    async def _reenter_one(sid: str, strat_exec) -> List[InstanceResult]:
        ...
        try:
            ...
            res = []
        state_value = (
            "completed" if any(getattr(r, "success", False) for r in (res or [])) else "failed"
        )
        ...
        if status == "failed":
            try:
                any_interrupted = any(
                    (
                        orchestrator.state_manager.current_state.instances.get(iid).state.value
                        == "interrupted"
                    )
                    for iid in (orchestrator.state_manager.current_state.strategies.get(sid).instance_ids or [])
    ...
    for sid, strat_exec in list(orchestrator.state_manager.current_state.strategies.items()):
...
async def resume_run(orchestrator, run_id: str) -> List[InstanceResult]:
    ...
    _backfill_terminal(orchestrator, run_id)
    _reopen_strategies(orchestrator, run_id)
    await orchestrator.state_manager.start_periodic_snapshots()

    scheduled_ids = await _schedule_instances(orchestrator, saved_state)
    if scheduled_ids:
        logger.info("resume_run: enqueued %s instance(s) for resume", len(scheduled_ids))

    reentry_results = await _reenter_strategies(orchestrator, run_id)

    try:
        if orchestrator.state_manager and orchestrator.state_manager.current_state:
            orchestrator.state_manager.current_state.completed_at = datetime.now(timezone.utc)
    except Exception:
        pass
