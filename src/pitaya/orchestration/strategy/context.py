...
from pitaya.orchestration.strategy.handles import Handle, InstanceHandle
from pitaya.orchestration.strategy.random import DeterministicRand
from pitaya.orchestration.strategy.task import _schedule_async
...
class StrategyContext:
    ...
    async def wait(self, handle: Handle) -> InstanceResult:
        ...
        if not getattr(r, "success", False):
            try:
                from pitaya.exceptions import TaskFailed
    ...
    async def wait_all(
        ...
        if any(not getattr(r, "success", False) for r in out):
            try:
                from pitaya.exceptions import AggregateTaskFailed
