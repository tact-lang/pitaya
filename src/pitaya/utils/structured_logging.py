...
_COMPONENT_TO_MODULES: Dict[str, Tuple[str, ...]] = {
    "orchestration.jsonl": ("pitaya.orchestration",),
    "runner.jsonl": ("pitaya.runner",),
    "tui.jsonl": ("pitaya.tui",),
}
...
def get_component_logger(component: str) -> logging.Logger:
    """Return logger name-spaced under ``pitaya.`` when missing the prefix."""
    if not component.startswith("pitaya."):
        component = f"pitaya.{component}"
...
def _configure_existing_loggers(
    ...
    for logger_name in list(logging.Logger.manager.loggerDict.keys()):
        if not logger_name.startswith("pitaya."):
            continue
